"""
Deep4Net Subject 1 跨会话独立评价与可视化。

本脚本执行：
1. 重新加载 BNCI2014_001 Subject 1；
2. 使用与训练阶段完全一致的预处理；
3. 加载验证准确率最高的 Deep4Net 模型；
4. 在 1test 会话上进行独立预测；
5. 计算 Accuracy、Balanced Accuracy 和 Macro-F1；
6. 生成分类报告和混淆矩阵；
7. 绘制训练准确率与损失曲线；
8. 保存逐样本预测结果。
...
输出文件：
    results/metrics/deep4net_subject1_evaluation.csv
    results/metrics/deep4net_subject1_classification_report.txt
    results/metrics/deep4net_subject1_predictions.csv

    results/figures/deep4net_subject1_confusion_matrix.png
    results/figures/deep4net_subject1_accuracy.png
    results/figures/deep4net_subject1_loss.png
"""
...
EVALUATION_PATH = (
    METRICS_DIR
    / "deep4net_subject1_evaluation.csv"
)

REPORT_PATH = (
    METRICS_DIR
    / "deep4net_subject1_classification_report.txt"
)

PREDICTIONS_PATH = (
    METRICS_DIR
    / "deep4net_subject1_predictions.csv"
)

CONFUSION_MATRIX_PATH = (
    FIGURES_DIR
    / "deep4net_subject1_confusion_matrix.png"
)

ACCURACY_CURVE_PATH = (
    FIGURES_DIR
    / "deep4net_subject1_accuracy.png"
)

LOSS_CURVE_PATH = (
    FIGURES_DIR
    / "deep4net_subject1_loss.png"
)
...
def create_deep4net() -> Deep4Net:
    """
    创建与训练阶段结构完全一致的 Deep4Net。
    """
    model = Deep4Net(
        n_chans=N_CHANS,
        n_outputs=N_CLASSES,
...
def collect_predictions(
    ...
    with torch.no_grad():
        for batch in data_loader:
            ...
            if outputs.ndim != 3:
                raise RuntimeError(
                    "Expected cropped model output with shape "
                    "(batch, classes, predictions), "
                    f"but received {tuple(outputs.shape)}."
                )

            # 对一个窗口内部的多个时间位置预测取平均。
            window_scores = outputs.mean(dim=2)
...
def plot_confusion_matrix(
    ...
    axis.set_title(
        "Deep4Net Subject 1 Confusion Matrix"
    )
...
def main() -> None:
    ...
    print("=" * 72)
    print("Deep4Net Subject 1 Independent Evaluation")
    print("=" * 72)
    ...
    print("\n[3/7] Loading best Deep4Net model...")

    model = create_deep4net()
    ...
    print(f"Validation windows: {len(valid_set)}")
